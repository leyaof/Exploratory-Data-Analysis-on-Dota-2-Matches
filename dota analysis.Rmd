...
Previously, we looked at the distribution of play counts across different time variables along with their win rates, in this section I will explore which heroes Jeff dedicated most of his time into.  
```{r}
...
```
There is a drastic increase in the play count with Gyrocopter in the years of 2018 and 2019. This along with the 80% win rate discovered previously explained why there was a linear increase in win rate from the line graph describing win rate by year earlier.   
...
team_wl <- ggplot() +
  ...
  ylim(0,1) +
  labs(x="team", y="percent won", title="win rate by year: Radiant vs Dire") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(legend.position="right") +
...
```
Since all three QQ plots have obvious curves, the data does not agree with the normality assumption, the Mood's median test would be a better fit for this case. The null hypothesis is that there is no difference between the two groups.   
...
Based on the above evidence, I conclude that there is no difference between the groups Radiant and Dire in terms of kills, deaths or assists. However, we previously discovered that the win rate of being on the Radiant team is 4.31% than being on the Dire team. Therefore, there may be other influential factors contributed to the win rate that was not recorded in the dataset used in this project.       
...
* The lifetime win rate is attempted to be balanced at 50%
